...
[![Build: MSVC | GCC | Clang](https://img.shields.io/badge/Compiler-MSVC%20%7C%20GCC%20%7C%20Clang-FF6F00.svg?style=flat-square)]()
...
#### Using MSVC (Visual Studio Developer Command Prompt on Windows)
```cmd
cd Tensor
...
#### Using GCC / Clang (Linux / macOS / MinGW)
```bash
cd Tensor
...
| 运算分类 | 支持的函数列表 | 自动微分与梯度传播 |
| :--- | :--- | :---: |
| **基础代数与广播** | `add`, `sub`, `mul`, `div`, `neg`, `pow`, `exp`, `log` | ✅ 完全支持反向求导 |
| **三角函数与双曲** | `sin`, `cos`, `tan`, `tanh` | ✅ 完全支持反向求导 |
| **神经网路激活函数** | `relu`, `sigmoid`, `softmax` | ✅ 完全支持反向求导 |
| **线性代数与矩阵** | `matmul` (矩阵乘法), `dot` (点积), `transpose` (转置), `inverse` (求逆) | ✅ 完全支持反向求导 |
| **张量降维聚合** | `sum` (求和), `mean` (求均值) | ✅ 完全支持反向求导 |

---

### 💻 编译与运行指南

#### 在 Windows 环境中使用 MSVC 编译器 (Developer Command Prompt)
```cmd
cd Tensor
cl /EHsc /std:c++17 main.cpp src\Tensor.cpp src\ops\*.cpp /Fe:main.exe
main.exe
...
#### 使用 GCC / Clang 编译器 (Linux / macOS / MinGW)
```bash
cd Tensor
g++ -std=c++17 main.cpp src/Tensor.cpp src/ops/*.cpp -o main.exe
./main.exe
